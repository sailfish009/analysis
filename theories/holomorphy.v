...
Lemma real_complexA (x : R) : x%:C%:A = x%:C :> C^o.
Proof.
by rewrite eqE_complex /=; split; rewrite ?mulr0 ?mul0r ?mulr1 ?addr0 ?subr0.
Qed.
...
Lemma gt0_real_complex (r : C) : 0 < r -> r = (Re r)%:C.
Proof.
...
Lemma complex_pos (r : {posnum C}) :  0 < (complex.Re r%:num).
Proof.  by rewrite -ltr0c -gt0_real_complex. Qed.

Lemma complex_gt0E (d : C) :  0 < d = (0 < complex.Re d :> R). (*name*)
Proof. Admitted.

Lemma complex_gtE (c d : C) :  c < d = (complex.Re c < complex.Re d). (*name*)
Proof. Admitted.

Canonical complex_Re_posnum (x : {posnum C}) := PosNum (complex_pos x).

Lemma real_complex_gt0 (r : {posnum R}) :  0 < (r%:num)%:C.
Proof. by rewrite ltcR. Qed.

Canonical real_complex_posnum (x : {posnum R}) := PosNum (real_complex_gt0 x).
...
Definition holomorphic (f : C^o -> C^o) (c : C^o) :=
  cvg ((fun (h : C^o) => h^-1 *: ((f \o shift c) h - f c)) @ (nbhs' (0:C^o))).
...
Fail Definition Rderivable_fromcomplex_false (f : C^o -> C^o) (c v: C^o) :=
  cvg ((fun (h : R^o) =>  h^-1 *: ((f \o shift c) (h *: v) - f c)) @ (nbhs' (0:R^o))).


Definition real_complex : R^o -> C := ( fun r => r%:C ).
(*make shorter *)

Definition Rderivable_fromcomplex (f : C^o -> C^o) (c v: C^o) :=
  cvg ((fun (h : C^o) => h^-1 * ((f \o shift c) (h *:v) - f c)) @
                                     (real_complex @ (nbhs' (0:R^o)))).
...
case: (v =P 0) =>  [eqv0|/eqP vneq0].
  have -> :  quotR \o real_complex = 0.
    ...
    by rewrite scaler0 add0r subrr mulr0 //=.
  by apply: (cvgP (0:C^o)); apply: (cvg_cst (0 : C^o)).
apply: (cvgP (v *: l)).
have eqnear0: {near nbhs' 0, (v \*o quotC) \o mulrv =1 (quotR \o real_complex)}.
(* as we need to perform computation for h != 0, we cannot prove fun equality directly *)
  exists 1 => // h _ neq0h; rewrite /quotC /quotR /real_complex /mulrv //=.
(*have -> :  (h *: v)^-1 = h^-1 *: v^-1. Fail rewrite (@invrZ R _ h _). *) (* issue with R^o*)
  rewrite invrM ?unitfE //; last by rewrite neqCr0 ?unitfE.
  by rewrite mulrA (mulrA v) mulrV ?unitfE // mul1r.
have subsetfilters : quotR \o real_complex @ nbhs' 0 `=>` (v \*o quotC) \o mulrv @ nbhs' 0.
...
set quotC := (X in (exists l : ((R[i])^o)^o,  X @ nbhs' 0  --> l)).
simpl in (type of quotC).
set quotR := fun h => h^-1 *: ((f \o shift c) (h * 'i) - f c) .
simpl in (type of quotR).
...
have lem : quotC \o  *%R^~ 'i%R @ (real_complex @ (nbhs' (0 : R^o))) --> l.
  ...
  by rewrite eqCr.
have HRcomp:  cvg (quotC \o *%R^~ 'i%R @ (real_complex @ (nbhs' (0 : R^o)))) .
  by apply/cvg_ex;  simpl; exists l.
have ->: lim (quotR @ (real_complex @ (nbhs' (0 : R^o))))
  = 'i *: lim (quotC \o ( fun h => h *'i) @ (real_complex @ (nbhs' (0 : R^o)))).
...
rewrite scalecM.
suff: lim (quotC @ (real_complex @ (nbhs' (0 : R^o))))
      = lim (quotC \o  *%R^~ 'i%R @ (real_complex @ (nbhs' (0 : R^o)))) by move => -> .
suff -> : lim (quotC @ (real_complex @ (nbhs' (0 : R^o)))) = l.
  apply/eqP; rewrite eq_sym; apply/eqP. apply: (cvg_map_lim _ lem).
  by apply: norm_hausdorff.
by apply: (@cvg_map_lim ((numFieldType_topologicalType (complex_numFieldType R))));
first by apply: norm_hausdorff.
Qed.
...
Section Rcomplex.
  (* To prove that CaucyRieman -> holomorphic, we want to use the theory of
R-differentiable functions on C. The following is a first naïve approach *)
  (* Define Rcomplex on Num.real instead of R ? *)
...
Lemma real_complexZ (a : R) (b : Rcomplex) : a%:C * b = a *: b.
Proof. rewrite eqE_complex //=; split.
Admitted.
...
Lemma complex_cvgP (F : set (set C)) (FF: Filter F) : [cvg F in Rcomplex_normedModType R] <->
   [cvg F in ((numFieldType_filteredType  (complex_numFieldType R)))].
Proof.
split; move/cvg_ex => /= [l H].
apply: (cvgP (l : (numFieldType_filteredType (complex_numFieldType R)))).
  by apply/complex_limP.
by apply: (cvgP (l : Rcomplex_filteredType R)); apply/complex_limP.
Qed.


Lemma complex_liminP (F : set (set C)) (FF: ProperFilter F):
   [lim F in Rcomplex_normedModType R] =
   [lim F in ((numFieldType_filteredType  (complex_numFieldType R)))].
Proof.
case: (EM ([cvg F in ((numFieldType_filteredType  (complex_numFieldType R)))])).
  move/cvg_ex=> /= [l Fl].
  rewrite (@norm_cvg_lim _ (numFieldType_normedModType _)   _ _ l) //.
...
have -> :  (fun h : (R[i])^o => h^-1 * ((f \o shift c) (h *: v) - f c)) \o
  ...
  by apply: funext => h /=; rewrite real_complex_inv -scalecM -!scalecr.
...
Lemma Rdiff1 (f : C^o -> C^o) c:
          lim ( (fun h : C^o => h^-1 *: ((f \o shift c) (h) - f c))
                 @ (((@real_complex R))@ (nbhs' (0:R^o))))
         = 'D_1 (f%:Rfun) c%:Rc.
Proof.
rewrite /derive lim_compE.
suff -> :  (fun h : (R[i])^o => h^-1 * ((f \o shift c) (h ) - f c)) \o
real_complex (R:=R) = fun h : R => h^-1 *: ((f%:Rfun \o shift c) (h *: (1%:Rc)) - f c).
  by rewrite complex_liminP /=.
apply: funext => h /=.
by rewrite real_complex_inv -scalecM -!scalecr real_complex_alg.
Qed.
...
real_complex (R:=R) = fun h : R => h^-1 *: ((f%:Rfun \o shift c) (h *: ('i%:Rc)) - f c).
  by rewrite complex_liminP /=.
apply: funext => h /=.
by rewrite real_complex_inv -scalecM -!scalecr real_complexZ.
Qed.
...
have lem : forall (h : C^o), h%:A = h.
by move => h; rewrite [h *: 1]/(h *1) //=; apply: mulr1.
suff -> : (fun h => h^-1 *: ((f \o shift c) h - f c)) =
             ((fun h  => h^-1 *: ((f \o shift c) h%:A - f c))) by []. (* time *)
by apply: funext => h;  rewrite lem.
Qed.
...
have er_lt0 : 0 < er/2 by rewrite mulr_gt0 // -?complex_gt0E //.
...
pose dr := complex.Re d.
have eq_drd := gt0_real_complex d0.
pose dr3 := (dr/2%:R).
pose dr' := (dr/2%:R/2%:R).
